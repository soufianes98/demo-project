...
check_git_tags() {
    ...
    # `git tag -l "v*"` command lists all tags that starts with `v`
    if [ -z "$(git tag -l "v*")" ]; then
        ...
        # Get previous/latest git tag
        # Remove "v" prefix with empty string `sed 's/^v//'`
        # Try `echo "v1.0.0" | sed 's/^v//'` to test this snippet and it should outputs like this value 1.0.0
...
get_latest_commits_list() {
    ...
    # List all commits with commit hash
    ...
    if [ -n "$latest_tag" ]; then
        ...
        done < <(git log "v$latest_tag"..HEAD --pretty=format:"%h,%s,%b")
    else
        while IFS= read -r commit; do
            # Push one item to commits_array
            commits_array+=("$commit")
            # Capturing the output of git log
            # and feed the output to the while loop
...
process_commit() {
    ...
    # TODO: Here we handle breaking changes

    # Here we check if anything before the first `:` in the commit_subject ends with `!`
    # then it means a breaking change
...
parse_latest_commits() {
    ...
    log "Found ${#commits_array[@]} commits"

    # Iteration over commits_array values
    log "Iteration over the latest commits"
    for commit in "${commits_array[@]}"; do
...
parse_latest_tag() {
    ...
    # Split the latest_tag into major, minor and patch using `.` as the delimiter
...
bump_version() {
    ...
    if [[ "$is_first_release" = true ]]; then
        ...
        read -r major minor patch <<<"$(parse_latest_tag "$latest_tag")"
        ...
        if [[ ${#breaking_changes[@]} -ne 0 ]]; then
            ...
            next_tag="$((++major)).0.0"
            log "Version bumped to: v$next_tag"
            is_pre_release=false

            return # Exit/Break this function and move to the next function
        fi
    ...
    log "No changes in your codebase!"
...
create_github_release() {
    ...
    for artifact_path in "${artifacts_paths[@]}"; do
        ...
        curl -L \
            ...
            "https://uploads.github.com/repos/$USERNAME/$REPOSITORY_NAME/releases/$RELEASE_ID/assets?name=$artifact_filename"
        # || {
